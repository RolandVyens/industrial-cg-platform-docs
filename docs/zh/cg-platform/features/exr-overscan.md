...
- **支持深度和多层格式** — 与所有的 EXR 格式完美集成，包括多层 EXR（OpenEXR Multilayer）以及深度 EXR（Deep EXR）渲染。
...
当导入到合成软件中时，文件会自动对齐到项目的 1920x1080 交付格式，而视口外的溢画幅像素将被保留在 Bounding Box（BBox）中，供后期反畸变和防抖使用。
...
- [RNA 属性 (API 参考)](/zh/cg-platform/api/rna-properties) — 用于通过 Python 脚本配置溢画幅的 API 属性。
- [安装指南](/zh/cg-platform/guide/installation) — 关于 OptiX 着色器缓存隔离的详细信息。
